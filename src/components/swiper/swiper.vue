...
<template>
  <div class="swiperHome" v-if="isAjax">
    <swiper :options="swiperOption">
      <swiper-slide v-if="matchList.length >= 1" v-for="(item, index) in matchList" :key="index">
        <div class="swiperUntil">
          <div class="topB">
            ...
            <div class="prize">
              <div class="prizeTextName">免费赢取</div>
              <div class="prizeText" v-if="item.prizeInfo[0]">{{item.prizeInfo[0].prizeMoney | formatInt}}{{item.prizeInfo[0].goodsName}}</div>
            </div>
          </div>
          ...
          <div class="bottom" @click="goGamePage(item)">
            立即参与
          </div>
      ...
      <swiper-slide v-if="matchList.length === 0">
        <div class="swiperUntil">
          <div class="topB">
...
<script>
  ...
  export default {
    ...
    mounted () {
      console.log(this.matchList.length, 'mathlist')
    },
    methods: {
      // 解锁参加
      sureJoin (item) {
